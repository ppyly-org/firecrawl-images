...
This repository pins the source used to build private Firecrawl images. The pin is deliberately split into `UPSTREAM_VERSION` (`v2.11.0`) and `UPSTREAM_COMMIT` (`ef12eb36b2f3382838dfe0a0c1a5add3d5df7fe5`). `vendor/firecrawl` is a detached, ignored checkout; it is fetched locally and is never committed.
...
Upstream `v2.11.0` is an unsigned lightweight tag: its GitHub tag ref is a `commit`, rather than a signed annotated tag. GitHub also reported the release as mutable, and the same commit is tagged `v2.10.30`. The exact SHA pin makes the local checkout reproducible and prevents a moved tag from changing it, but it is not cryptographic release verification. Both scripts explicitly require the local tag object to be a commit and require its peeled commit and `HEAD` to equal `UPSTREAM_COMMIT`.

## Build contexts for future image work

The API/harness Dockerfile must use the Firecrawl repository root as its context because it copies root workspace files and `sharedLibs/go-html-to-md`:

```bash
docker build --file vendor/firecrawl/apps/api/Dockerfile vendor/firecrawl
```

The Playwright service Dockerfile is self-contained and must use its service directory as its context:

```bash
docker build --file vendor/firecrawl/apps/playwright-service-ts/Dockerfile \
  vendor/firecrawl/apps/playwright-service-ts
```
